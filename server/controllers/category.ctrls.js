...
export const deleteCategory = async (req, res) => {
    ...
    try {
        ...
        if (category.countryCode !== req.user.countryCode) {
            return res.status(403).json({ message: 'Cannot delete category from another country' });
        }
...
export const updateCategory = async (req, res) => {
    ...
    try {
        ...
        if (category.countryCode !== req.user.countryCode) {
            return res.status(403).json({ message: 'Cannot update category from another country' });
        }
...
export const createGroup = async (req, res) => {
  ...
  try {
    ...
    if (category.countryCode !== req.user.countryCode) {
      return res.status(403).json({ message: 'Cannot create group in another country\'s category' });
    }
...
export const updateGroup = async (req, res) => {
    ...
    try {
        const group = await LocalGroupModel.findById(groupId).populate('categoryId');
        if (!group) return res.status(404).json({ message: 'Group not found' });

        if (group.categoryId.countryCode !== req.user.countryCode) {
            return res.status(403).json({ message: 'Cannot update group in another country\'s category' });
        }
...
export const deleteGroup = async (req, res) => {
    ...
    try {
        ...
        if (group.categoryId.countryCode !== req.user.countryCode) {
            return res.status(403).json({ message: 'Cannot delete group in another country\'s category' });
        }
