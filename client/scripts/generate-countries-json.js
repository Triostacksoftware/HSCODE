...
// Get first letter of country name for filtering
const getFirstLetter = (name) => {
  const firstChar = name.charAt(0).toUpperCase();
  return /[A-Z]/.test(firstChar) ? firstChar : 'A';
};

// Read flag images directory
const flagImagesDir = path.join(__dirname, '../public/flag_images');
const flagFiles = fs.readdirSync(flagImagesDir).filter(file => 
  file.endsWith('.svg') && !file.startsWith('gb-') // Exclude GB sub-regions
);

// Generate countries array
const countries = flagFiles.map(file => {
  ...
  // Sort by letter first, then by name
