...
export const HSCodeProvider = ({ children }) => {
  ...
  const parseCSV = useCallback((csvText, country) => {
    try {
      ...
      const hasHeader = firstLine.includes('hscode') || firstLine.includes('code') || firstLine.includes('hs');
  ...
  const loadHSCodes = useCallback(async (userCountryCode) => {
    ...
    try {
      ...
      console.log(`Loaded ${allCodes.length} total HS codes from ${userCountryCode} and US`);
    } catch (error) {
